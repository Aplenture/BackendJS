...
export class DeleteAccess extends Command<Context, Args, Options> {
    ...
    public readonly parameters = new CoreJS.ParameterList(
        ...
        new CoreJS.StringParameter('api_to_delete', "API of the access to delete.", null)
    );

    public async execute(args: Args): Promise<CoreJS.Response> {
        if (args.api_to_delete) {
            ...
            await this.context.accessRepository.deleteByAccount(args.account, args.access);

            this.message(`deleted all accesses of account '${args.account}'`);
        }
