...
describe.only("Balance Repository", () => {
    ...
    describe("Events", () => {
        describe("get", () => {
            ...
            it("with data", () => repository.getEvents(1, { data: 'decrease' }).then(result => expect(result.map(data => data.value)).has.length(4).deep.equals([6, 7, 8, 9])));
        ...
        describe("fetch", () => {
            ...
            it("with data", async () => { const array = new Array<Balance.Event>; await repository.fetchEvents(1, async data => array.push(data), { data: 'decrease' }).then(() => expect(array.map(data => data.value)).has.length(4).deep.equals([6, 7, 8, 9])) });
